...
PLOTS=[
 ('building_warehouse',-23,53,4.2,23,19,0),('building_warehouse_b',-76,53,4.2,17,22,0),
 ('building_harbor_office',12,62,4.2,20,19,.03),
 ('building_merchant_a',-24,78,6.4,17,18,0),('building_merchant_b',-70,76,6.4,15,21,0),
...
def natural_height(x,z):
 ...
 axis=z-130-(x+45)*.14
 mass=24*math.exp(-((x+51)/79)**4-(axis/22)**2)
 saddle=5*math.exp(-((x+29)/15)**2-((z-130)/30)**2)
 ravine=5*math.exp(-((x+106+(z-114)*.55)/5)**2)*smooth(103,122,z)
 bedding=1.1*math.sin((z-x*.22)*.5)+.65*math.sin(x*.19+z*.12)
...
def terrain_height(x,z):
 ...
 for _,px,pz,y,w,d,_ in PLOTS+[('plaza',-46,78,6.4,23,18,0)]:
  ...
  h=h*(smooth(0,3,edge))+y*(1-smooth(0,3,edge))
 for a,b,w in ROADS:
