"""Task04 authored Hero Slice. Blender 4.5; metric source -> Y-up GLB.
...
for args in [('building_house_a',9,9,8,'home'),('building_house_b',11,8,9,'home'),('building_house_c',8,12,6,'home'),('building_warehouse',19,12,7,'warehouse'),('building_warehouse_b',12,15,8,'warehouse'),('building_governor',20,14,10,'hall'),('building_church',13,20,12,'church')]:building(*args)
...
cube('stern_gallery',(0,4.2,-12),(7.1,2.8,4.1),'waterblue',.18)
...
cube('raised_quarterdeck',(0,5.65,-12),(7.3,.2,4.5),'deck',.1)
...
for name,height,count in [('prop_tropical_tree',11,1500),('prop_bush',2.8,240)]:
    reset(name);rod('tapered_trunk',(0,0,0),(.4,height*.65,0),height*.032,'wood',16);vs=[];fs=[]
    for i in range(count):
        a=RNG.uniform(0,math.tau);rad=RNG.uniform(.1,1)**.4*height*.4;y=height*.58+height*.40*math.sqrt(max(0,1-(rad/(height*.4))**2))+RNG.uniform(-.045,.045)*height;c=(math.cos(a)*rad,y,math.sin(a)*rad)
    ...
    shades=[(.06,.14,.027),(.105,.20,.045),(.16,.255,.065),(.085,.17,.037)]
...
# Ground texture + vertex tint controls grass/sand/rock transitions without hard material seams.
material=M['ground'].copy();p=material.node_tree.nodes.get('Principled BSDF');color=material.node_tree.nodes.new('ShaderNodeVertexColor');color.layer_name='TerrainColor';material.node_tree.links.new(color.outputs['Color'],p.inputs['Base Color']);o.data.materials.clear();o.data.materials.append(material)
ca=o.data.color_attributes.new(name='TerrainColor',type='FLOAT_COLOR',domain='CORNER')
for poly in o.data.polygons:
    poly.use_smooth=True
    for li in poly.loop_indices:
        x,h,z=vs[o.data.loops[li].vertex_index];g=np.clip((h-1.3)/2.4,0,1);rock=np.clip((h-13)/12,0,1)
        c=np.array((.57,.51,.37))*(1-g)+np.array((.16,.215,.095))*g;c=c*(1-rock)+np.array((.34,.33,.26))*rock
        mottled=.86+.09*math.sin(x*.42+z*.19)+.05*math.cos(z*.78-x*.31);ca.data[li].color=(*list(c*mottled),1)
...
for i in range(48):
    ...
    if h>3:place('prop_tropical_tree',x,z,heading=float(RNG.uniform(0,6)),shadow=False)
for i in range(55):
...
manifest['textures']=['/assets/textures/'+im.name+'.png' for im in textures.values()]
manifest['quality']='Task04 Hero Slice; LOD0; authored PBR surfaces and geometric leaf blades'
