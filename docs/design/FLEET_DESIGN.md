...
当前一个权威舰队使用一艘sloop代表，界面名“晨曦号”为展示名称，尚无船名编辑/多船编队。航行朝向取snapshot.heading；停泊时Rendering采用0.8弧度展示帆面，snapshot仍保持原规则。轻微摆动和尾流使用模拟时间，暂停冻结动画，镜头仍可移动。选择圈与方向箭头仅作UI提示，目的航线使用虚线。

## Task04 显示更新

单一Fleet仍是权威导航单位。可见模型升级为双桅BRIG，沿用sloop资产路径，不新增Fleet/Ship业务系统。船体水线以下约2.4米。选择改为分段方位弧，前向提示随航行heading，停泊展示角仍0.8弧度；尾流/摆动/路线脉动全属Rendering。
