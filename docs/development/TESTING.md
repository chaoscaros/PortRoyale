...
tests/navigation.test.ts新增20项（参数化计数）：四港唯一性/有效性/lookup、初始舰队、合法与非法命令、当前港no-op、直线距离、1/2/4位移、pause/resume、精确抵达及高速不越界、改道/重复目标/返回原港、帧分割确定性、深冻结与输入防别名、速度校验。合计2文件30项。核心不import Babylon/DOM。

浏览器顺序：四港显示→点击船高亮→目标港→前往→船首与移动→倍率/暂停/继续→抵达→中途改道。补Phase0相机完整检查。页面无法连接时标记PENDING，不自行启动服务。

## Task 03

自动检查：3文件32测试通过；新增资产交付测试检查真实GLB结构/三角数/引用，不重复业务实现。typecheck通过（含无DOM核心）。build通过，lint未配置。运行细项见AI_HANDOFF与DEVLOG，不能由静态检查推定视觉质量。
