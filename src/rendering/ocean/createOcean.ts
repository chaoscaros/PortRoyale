...
export function createOcean(scene: Scene, ports: readonly PortSnapshot[]) {
  ...
  const material = new ShaderMaterial(
    ...
    float waves(vec2 q){vec2 drift=vec2(time*.045,-time*.035);float swell=sin(q.x*.095+q.y*.061+time*.31)*.10+sin(q.y*.131-q.x*.077-time*.27)*.07;return swell+fbm(q*.43+drift)*.23+fbm(q*1.2-drift*1.3)*.055;}
    ...
    float sandbar=fbm(q*.025);vec3 water=mix(vec3(.021,.135,.175),vec3(.14,.39,.32),shallow*(.82+sandbar*.18));water+=vec3(.024,.04,.028)*(fbm(q*.017)-.5);
